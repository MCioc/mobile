...
$(function(){
	$.get("getfee.do",{charge:"A"},function(data){
		$("#txt_fee").val(data);
	})
	
	$("#sel_charge").change(function(){
		$.get("getfee.do",{charge:$(this).val()},function(data){
			$("#txt_fee").val(data);
		})
	})		
})
...
<h2>收费细项</h2>
    <form method="POST" action="chargeset.do">
      <p>收费项目：<select size="1" name="charge" id="sel_charge">
                     <c:forEach items="${list}" var="c">
                       <option value="${c.charge_Code}">${c.charge_Name}</option>
                     </c:forEach>      
                  </select></p>
      <p>收费金额：<input type="text" name="fee" size="20"  id="txt_fee"></p>
      <p><input type="submit" value="提交" name="B1"><input type="reset" value="全部重写" name="B2"></p>
    </form>
    <hr>
    <h2>业务收费</h2>
    <form method="POST" action="chargeruleadd.do">
      <p>业务：<select size="1" name="D1" id="sel_cr">  
        <option value="开户业务">开户业务</option>
      </select></p>
      <p>收费项目：
                <c:forEach items="${list}" var="c">
                   <input type="checkbox" name="C" value="${c.charge_Code}" >${c.charge_Name}
                </c:forEach>
      </p>
      <p><input type="submit" value="提交" name="B1"><input type="reset" value="全部重写" name="B2"></p>
    </form>   
